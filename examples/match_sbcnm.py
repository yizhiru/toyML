...
from toyml.match import SBCNMNetwork
from toyml.match import sbcnm_loss

flags.DEFINE_string(
    "train_path", "train.tfrecord", "Input file path used for training."
)
flags.DEFINE_string("eval_path", "test.tfrecord", "Input file path used for eval.")
flags.DEFINE_string("model_dir", "sbcnm_model", "Output directory for models.")
...
flags.DEFINE_integer(
    "list_size", 15, "List size used for training. Use None for dynamic list size."
)
...
def _create_feature_columns():
    ...
    def _sequence_fc(sequence_feature: SequenceFeature):
        return fc.numeric_column(
            key=sequence_feature.feature_name,
    ...
    ) = _create_match_features()

    context_feature_columns = _all_fc(
...
def _create_match_features():
    ...
    context_sequence_features = [
        SequenceFeature(
            "tag_prefer_list", 10, SparseFeature("hash_tag_id", 10000, _EMBEDDING_DIM)
        ),
        SequenceFeature(
            "click_feed_hist", 60, SparseFeature("hash_feed_id", 100000, _EMBEDDING_DIM)
        ),
    ]
...
def make_dataset(file_pattern, batch_size, randomize_input=True, num_epochs=1):
    ...
    context_feature_spec = fc.make_parse_example_spec(context_feature_columns.values())
    label_column = tf.feature_column.numeric_column(
        _LABEL, dtype=tf.int64, default_value=-1
    )
...
def train_and_eval():
    ...
    metrics = [
        PrecisionMetric(name="precision@1", topn=1),
